      ...
      for(std::vector<int>::const_iterator it=norder.begin();it!=norder.end();++it){
        real_t worst_q;
        {
          typename std::set<index_t>::iterator ie=NEList[*it].begin();
          {
            ...
            worst_q = functional.calculate(x0, x1, x2, x3,
          ...
          for(;ie!=NEList[*it].end();++ie){
            ...
            real_t x2[] = {_x[n[2]], _y[n[2]], _z[n[2]]};
            real_t x3[] = {_x[n[3]], _y[n[3]], _z[n[3]]};
            worst_q = min(functional.calculate(x0, x1, x2, x3,
                                               _metric+n[0]*9, _metric+n[1]*9, _metric+n[2]*9, _metric+n[3]*9), worst_q);
          }
        }
        ...
        // Want to solve the system Ap=q to find the new position, p.
        real_t p[] = {(-(((A01*A02/A00 - A12)*A01/((pow(A01, 2)/A00 - A11)*A00) - A02/A00)*(A01*A02/A00 - A12)/((pow(A01, 2)/A00 - A11)*(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22)) - A01/((pow(A01, 2)/A00 - A11)*A00))*q1 + (pow((A01*A02/A00 - A12)*A01/((pow(A01, 2)/A00 - A11)*A00) - A02/A00, 2)/(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22) - pow(A01, 2)/((pow(A01, 2)/A00 - A11)*pow(A00, 2)) + 1/A00)*q0 + ((A01*A02/A00 - A12)*A01/((pow(A01, 2)/A00 - A11)*A00) - A02/A00)*q2/(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22)),
                      ((pow(A01*A02/A00 - A12, 2)/(pow(pow(A01, 2)/A00 - A11, 2)*(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22)) - 1/(pow(A01, 2)/A00 - A11))*q1 - (((A01*A02/A00 - A12)*A01/((pow(A01, 2)/A00 - A11)*A00) - A02/A00)*(A01*A02/A00 - A12)/((pow(A01, 2)/A00 - A11)*(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22)) - A01/((pow(A01, 2)/A00 - A11)*A00))*q0 - (A01*A02/A00 - A12)*q2/((pow(A01, 2)/A00 - A11)*(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22))),
                      (((A01*A02/A00 - A12)*A01/((pow(A01, 2)/A00 - A11)*A00) - A02/A00)*q0/(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22) - (A01*A02/A00 - A12)*q1/((pow(A01, 2)/A00 - A11)*(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22)) + q2/(pow(A01*A02/A00 - A12, 2)/(pow(A01, 2)/A00 - A11) - pow(A02, 2)/A00 + A22))};
        ...
        real_t mp[9], l[4];
        int best_e=*NEList[*it].begin();
        for(size_t b=0;b<5;b++){ // 5 bisections along the search line
          real_t tol=-1;
          for(typename std::set<index_t>::iterator ie=NEList[*it].begin();ie!=NEList[*it].end();++ie){
            const index_t *n=_ENList+(*ie)*4;
            for(size_t i=0;i<4;i++){
              real_t x1[] = {_x[n[(i+1)%4]], _y[n[(i+1)%4]], _z[n[(i+1)%4]]};
              real_t x2[] = {_x[n[(i+2)%4]], _y[n[(i+2)%4]], _z[n[(i+2)%4]]};
              real_t x3[] = {_x[n[(i+3)%4]], _y[n[(i+3)%4]], _z[n[(i+3)%4]]};
              if(i==0)
                l[i] = functional.volume(p, x1, x3, x2);
              else if(i==1)
                l[i] = functional.volume(p, x1, x2, x3);
              else if(i==2)
                l[i] = functional.volume(p, x2, x1, x3);
              else if(i==3)
                l[i] = functional.volume(p, x1, x2, x3);
            }
            real_t min_l = min(l[0], min(l[1], l[2]));
          ...
          continue;
        {
          ...
          for(size_t i=0;i<9;i++){
            mp[i] = (l[0]*_metric[n[0]*9+i]+l[1]*_metric[n[1]*9+i]+l[2]*_metric[n[2]*9+i]+l[3]*_metric[n[3]*9+i])/L;
          }
        }
        ...
        for(typename std::set<index_t>::iterator ie=NEList[*it].begin();ie!=NEList[*it].end();++ie){
          ...
          real_t x1[] = {_x[n[loc1]], _y[n[loc1]], _z[n[loc1]]};
          real_t x2[] = {_x[n[loc2]], _y[n[loc2]], _z[n[loc2]]};
          real_t x3[] = {_x[n[loc3]], _y[n[loc3]], _z[n[loc3]]};

          real_t fxnl = 0;
          if(iloc==0)
            fxnl = functional.calculate(p, x1, x3, x2, mp, _metric+n[loc1]*9, _metric+n[loc2]*9, _metric+n[loc3]*9);
          else if(iloc==1)
            fxnl = functional.calculate(p, x1, x2, x3, mp, _metric+n[loc1]*9, _metric+n[loc2]*9, _metric+n[loc3]*9);
          else if(iloc==2)
            fxnl = functional.calculate(p, x2, x1, x3, mp, _metric+n[loc1]*9, _metric+n[loc2]*9, _metric+n[loc3]*9);
          else if(iloc==3)
            fxnl = functional.calculate(p, x1, x2, x3, mp, _metric+n[loc1]*9, _metric+n[loc2]*9, _metric+n[loc3]*9);
